...
const contentTypes = ["Social Video", "LinkedIn Post", "Substack Topic"];
...
export default function DashboardContentIdeas({ initialIdeas }: Props) {
  const router = useRouter();
  const [ideas, setIdeas] = useState<DashboardContentIdea[]>(initialIdeas);
  const [title, setTitle] = useState("");
  const [contentType, setContentType] = useState("Social Video");
  ...
  async function loadOrderedIdeas() {
    ...
    if (error) {
      console.error("Unable to load ordered dashboard ideas:", error);
      return;
    }
  ...
  async function getNextSortOrder() {
    ...
    return typeof currentFirst === "number" ? currentFirst - 1 : 0;
  }
  ...
  async function addIdea(event: FormEvent<HTMLFormElement>) {
    ...
    if (error || !data) {
      setErrorMessage(error?.message || "Unable to add the content idea.");
      setSaving(false);
      return;
    }
  ...
  return (
    <>
      <style jsx global>{`
        div.relative.z-10.space-y-7:has([data-dashboard-content-ideas]) {
          display: flex;
      ...
      <section
        ...
        className="relative overflow-hidden rounded-[28px] border border-white/80 bg-[linear-gradient(145deg,rgba(242,247,239,0.96),rgba(255,255,255,0.78))] p-6 shadow-[0_18px_55px_rgba(71,91,66,0.11)] backdrop-blur-2xl lg:p-7"
      >
        ...
        <div className="relative">
          ...
          {ideas.length === 0 ? (
            ...
            <div className="mt-5 divide-y divide-[#e8ede5] overflow-hidden rounded-2xl border border-white/85 bg-white/62">
              {ideas.map((idea) => (
                <div key={idea.id} className="flex items-center gap-3 px-4 py-3.5">
                  <button
                    type="button"
