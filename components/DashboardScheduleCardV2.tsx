...
function Row({ meeting, past = false }: { meeting: Meeting; past?: boolean }) {
  return (
    <Link
...
export default function DashboardScheduleCardV2() {
  ...
  useEffect(() => {
    const heading = Array.from(document.querySelectorAll("h3")).find(
      (el) => el.textContent?.trim() === "People to Reach Out To"
    );
    const card = heading?.closest("div.rounded-\\[28px\\]") || heading?.parentElement?.parentElement?.parentElement;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await fetch(`/api/dashboard/upcoming-meetings?t=${Date.now()}`, {
          cache: "no-store",
  ...
  return createPortal(
    <div>
      ...
      {loading ? (
        <div className="mt-6 rounded-2xl border border-[#d2ddcd] bg-white/70 p-6 text-center text-sm text-[#708075]">Loading schedule...</div>
      ) : (
        <>
          <div className="mt-6 space-y-3">
            {upcoming.length ? upcoming.map((meeting) => <Row key={meeting.id} meeting={meeting} />) : (
          ...
          <div className="mt-7 border-t border-[#cfd9c9] pt-6">
            ...
            <div className="mt-4 max-h-[320px] space-y-3 overflow-y-auto pr-1">
              {past.length ? past.map((meeting) => <Row key={meeting.id} meeting={meeting} past />) : (
