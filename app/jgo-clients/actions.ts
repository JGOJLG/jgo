...
export async function updateJgoClientRow(formData: FormData) {
  ...
  const paymentStatus =
    amountReceived <= 0
      ? "Open"
      : amountOwed > 0 && amountReceived >= amountOwed
        ? "Paid"
        : "Partial";
  ...
  if (amountReceived > 0) {
    const extra = Math.max(0, amountReceived - amountOwed);
    const noteParts = ["Synced from JGO Clients spreadsheet."];
    if (extra > 0) noteParts.push(`Extra payment: $${extra.toFixed(2)}.`);
