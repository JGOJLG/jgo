...
function appliedPayment(row: JgoClientRow) {
  return Math.min(
    Math.max(Number(row.amount_received || 0), 0),
    Math.max(Number(row.amount_owed || 0), 0),
  );
}
...
export default function JgoClientsTracker({ initialRows }: { initialRows: JgoClientRow[] }) {
  ...
  function updateLocal(id: number, field: keyof JgoClientRow, value: string | number | boolean) {
    setRows((current) =>
      current.map((row) => (row.service_id === id ? { ...row, [field]: value } : row)),
    );
  }
  ...
  function archiveRow(row: JgoClientRow) {
    if (
      !window.confirm(
        `Archive ${row.client_name} - ${row.service}? It will be removed from this ledger but kept safely in JGO OS.`,
      )
    ) {
      return;
    }
    ...
    startTransition(async () => {
      try {
        // Save the current row first so archiving never discards an edit.
    ...
    <section className="min-w-0 flex-1 bg-[#f7f8f3] p-5 lg:p-8 xl:p-10">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[#708075]">
            Spreadsheet-style view of your real client records. Changes save to Supabase and sync back to the client profile, Revenue, and Outstanding.
          </p>
        </div>
        <div className="flex gap-2">
          <Link href="/clients" className="rounded-xl border border-[#cbd8c4] bg-white px-4 py-2.5 text-sm font-semibold text-[#4d6247]">
            Client Profiles
          </Link>
          <button onClick={() => setShowAdd((value) => !value)} className="rounded-xl bg-[#647d5b] px-4 py-2.5 text-sm font-semibold text-white">
            + Add Client Row
          </button>
        </div>
      </div>
      ...
      {showAdd ? (
        <form action={createJgoClientRow} onSubmit={() => setShowAdd(false)} className="mt-5 grid gap-3 rounded-2xl border border-[#dfe6db] bg-white p-4 md:grid-cols-[1.2fr_1fr_130px_150px_auto]">
          ...
          <input name="serviceDate" type="date" className="rounded-xl border border-[#dfe6db] px-2 py-2.5 text-xs" />
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-[#dfe6db] bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#7f8d82]">Total Invoiced</p>
          <p className="mt-2 text-2xl font-bold text-[#243128]">{currency(totals.owed)}</p>
        </div>
        <div className="rounded-2xl border border-[#dfe6db] bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#7f8d82]">Paid Toward Invoices</p>
          <p className="mt-2 text-2xl font-bold text-[#243128]">{currency(totals.paid)}</p>
          {totals.extra > 0 ? <p className="mt-1 text-xs font-semibold text-[#7f8d82]">+ {currency(totals.extra)} extra received</p> : null}
        </div>
        <div className="rounded-2xl border border-[#dfe6db] bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#7f8d82]">Outstanding</p>
          <p className="mt-2 text-2xl font-bold text-[#243128]">{currency(totals.outstanding)}</p>
        </div>
      </section>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-[#dfe6db] bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-[#dfe6db] bg-[#fbfaf6] px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#708075]">Edits auto-save. Archive keeps the record safely stored in JGO OS.</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[1080px]">
            <div className="grid grid-cols-[190px_92px_175px_105px_105px_115px_105px_105px_70px_38px] border-b border-[#dfe6db] bg-[#eef2ea] text-[9px] font-bold uppercase tracking-[0.08em] text-[#647066]">
              {['Client', 'Date', 'Service', 'Owed', 'Paid', 'Outstanding', 'Date Paid', 'Method', 'Profile', ''].map((label, index) => (
            ...
            {sortedRows.map((row, index) => (
              <div key={row.service_id} className={`grid grid-cols-[190px_92px_175px_105px_105px_115px_105px_105px_70px_38px] border-b border-[#edf0ea] ${index % 2 ? 'bg-[#fcfdfb]' : 'bg-white'}`}>
                ...
                <select value={row.payment_method || ''} onChange={(event) => { updateLocal(row.service_id, 'payment_method', event.target.value); save(row, { payment_method: event.target.value }); }} className={`${inputClass} border-r border-[#edf0ea]`}>
                ...
                <div className="flex items-center justify-center">
                  <button type="button" onClick={() => archiveRow(row)} className="flex h-6 w-6 items-center justify-center rounded-full text-sm font-medium text-[#a8aea8] transition hover:bg-[#f1f2ef] hover:text-[#7b847c]" aria-label={`Archive ${row.client_name}`} title="Archive">×</button>
                </div>
              </div>
            ))}
          </div>
...
function MoneyCell({ value, onSave }: { value: number; onSave: (value: number) => void }) {
  ...
  return (
    <input
      ...
      className="h-full w-full border-0 border-r border-[#edf0ea] bg-transparent px-2.5 py-2 text-right text-[13px] text-[#243128] outline-none focus:bg-white focus:shadow-[inset_0_0_0_2px_rgba(100,125,91,0.24)]"
    />
  );
}
