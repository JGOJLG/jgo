...
function getSignupId(formData: FormData) {
  const signupId = String(formData.get("signupId") || "").trim();
  if (!signupId) throw new Error("Invalid signup.");
  return signupId;
}

export async function markGuideSignupContacted(formData: FormData) {
  const signupId = getSignupId(formData);
...
export async function createLeadFromGuideSignup(formData: FormData) {
  ...
  const { data: signup, error: signupError } = await supabase
    .from("job_search_guide_leads")
    .select("id,first_name,email,created_at,converted_client_id")
    .eq("id", signupId)
    .single();
  ...
  if (existing?.id) {
    await supabase
      .from("job_search_guide_leads")
      .update({ funnel_status: "Converted to Lead", converted_client_id: existing.id })
      .eq("id", signupId);
    revalidatePath("/guide-signups");
  ...
  const { data: client, error: clientError } = await supabase
    .from("clients")
    .insert({
      name: signup.first_name?.trim() || normalizedEmail,
  ...
  await supabase
    .from("job_search_guide_leads")
    .update({ funnel_status: "Converted to Lead", converted_client_id: client.id })
    .eq("id", signupId);
