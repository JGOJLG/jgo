...
function easternWeekStartUtc(now = new Date()) {
  ...
  return new Date(
    Date.UTC(sy, sm - 1, sd, 0, 0) - offsetMinutes * 60_000
  );
}
...
function keepMeeting(event: RawMeeting) {
...
export async function GET() {
  ...
  if (pastResult.error) {
    return NextResponse.json(
      { error: pastResult.error.message },
      { status: 500, headers: { "Cache-Control": "no-store" } }
    );
  }

  const upcomingRaw = (upcomingResult.data ?? []).filter(keepMeeting);
  const pastRaw = (pastResult.data ?? []).filter(keepMeeting);
  ...
  const formatMeeting = (event: RawMeeting, isPast: boolean) => ({
    id: event.id,
  ...
  const meetings = upcomingRaw.map((event) => formatMeeting(event, false));
  const pastMeetings = pastRaw.map((event) => formatMeeting(event, true));
