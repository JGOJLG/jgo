...
export async function POST(request:Request){
 ...
 try{
  ...
  const{error:messageError}=await supabase.from("email_messages").insert({client_id:clientId,recipient_name:clientName,recipient_email:clientEmail,subject,body:messageText,body_html:messageHtml,status:"sent",template_id:null,sent_at:sentAt});
  if(messageError)console.error("Invoice sent but email history could not be saved",messageError);
  const{data:contact}=await supabase.from("email_contacts").select("id,email_count,first_contacted_at,name,client_id").eq("email",clientEmail).maybeSingle();
  if(contact)await supabase.from("email_contacts").update({name:clientName||contact.name||null,client_id:clientId,first_contacted_at:contact.first_contacted_at||sentAt,last_contacted_at:sentAt,email_count:Number(contact.email_count||0)+1,updated_at:sentAt}).eq("id",contact.id);
  else await supabase.from("email_contacts").insert({name:clientName,email:clientEmail,client_id:clientId,source:"jgo_os_invoice",first_contacted_at:sentAt,last_contacted_at:sentAt,email_count:1});
  console.info("JGO invoice accepted by Resend",{emailId:email.data?.id,recipient:clientEmail,cc,serviceId});return NextResponse.json({sent:true,accepted:true,tracked:!messageError,serviceId,checkoutUrl:checkout.url,emailId:email.data?.id??null,recipient:clientEmail,message:"Invoice accepted by email provider."});
