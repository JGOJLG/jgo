...
export default function ServiceCard({ clientId, service }: Props) {
  ...
  const [amountReceived, setAmountReceived] = useState(String(service.price ?? 0));
  ...
  const received = Number(service.amount_received ?? 0);
  ...
  function confirmDelete(event: React.FormEvent<HTMLFormElement>) {
    const confirmed = window.confirm(`Delete "${service.service}"? This cannot be undone.`);
    ...
    <div className="rounded-2xl border border-[#dfe6db] bg-[#fbfcf9] p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <button type="button" onClick={() => setShowEdit(true)} className="min-w-0 text-left">
          ...
          {service.date_added ? (
            <p className="mt-1 text-xs text-[#708075]">Added {new Date(`${service.date_added}T12:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
          ) : null}
        </button>
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {workStages.map((stage) => {
            ...
            return (
              <form key={stage} action={updateServiceStatus}>
                <input type="hidden" name="clientId" value={clientId} />
                <input type="hidden" name="serviceId" value={service.id} />
                <input type="hidden" name="status" value={stage} />
                <button type="submit" className={`rounded-full px-3.5 py-2 text-xs font-semibold transition ${active ? "bg-[#647d5b] text-white" : "border border-[#d7e1d0] bg-white text-[#647066] hover:border-[#9fb294] hover:text-[#3d4d39]"}`}>
                  {active ? "✓ " : ""}{stage}
                </button>
              </form>
            );
            ...
            <div className="mt-2">
              {isPaid ? (
                <div className="space-y-2">
                  ...
                  {received > 0 ? (
                    <div className="text-xs text-[#647066]">
                      <span className="font-semibold text-[#344239]">Received ${received.toLocaleString()}</span>
                      {extra > 0 ? <span className="ml-2 rounded-full bg-[#f4ead9] px-2 py-1 font-semibold text-[#8a6b3f]">+${extra.toLocaleString()} extra</span> : null}
                      {service.payment_method ? <span className="ml-2">via {service.payment_method}</span> : null}
                    </div>
                  ) : null}
                </div>
              ) : invoiceSent ? (
          ...
          <div className="flex flex-wrap gap-2">
            {!invoiceSent && !isPaid ? (
              <form action={markServiceInvoiceSent}>
                <input type="hidden" name="clientId" value={clientId} />
                <input type="hidden" name="serviceId" value={service.id} />
                <button type="submit" className="rounded-xl border border-[#cbd8c4] bg-white px-4 py-2 text-xs font-semibold text-[#4d6247] transition hover:bg-[#f5f7f2]">Mark Invoice Sent</button>
              </form>
            ) : null}
            {!isPaid ? (
              <button type="button" onClick={() => { setAmountReceived(String(service.price ?? 0)); setShowPayment(true); }} className="rounded-xl bg-[#647d5b] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#56683f]">Mark Paid</button>
            ) : null}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-[#243128]">Mark Payment Received</h3>
                <p className="mt-1 text-sm text-[#708075]">Invoice amount: ${invoiceAmount.toLocaleString()}</p>
              </div>
              <button type="button" onClick={() => setShowPayment(false)} className="text-[#708075] hover:text-[#243128]">✕</button>
            </div>
            ...
            <form action={markServicePaid} onSubmit={() => setShowPayment(false)} className="mt-5 space-y-4">
              ...
              <label className="block">
                ...
                <input required min="0" step="0.01" type="number" name="amountReceived" value={amountReceived} onChange={(e) => setAmountReceived(e.target.value)} className={inputStyle} />
              </label>

              {pendingExtra > 0 ? (
                <div className="rounded-xl border border-[#ead8b7] bg-[#fbf4e6] p-3 text-sm text-[#765b35]">
                  This includes <strong>${pendingExtra.toLocaleString()}</strong> above the ${invoiceAmount.toLocaleString()} invoice.
                </div>
              ) : null}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="block">
                  ...
                  <select name="paymentMethod" className={inputStyle} defaultValue="">
                    <option value="">Not specified</option>
                    <option>Venmo</option><option>Zelle</option><option>Card</option><option>Cash</option><option>Check</option><option>Other</option>
                  </select>
                </label>
                <label className="block">
                  <span className={labelStyle}>Date Paid</span>
                  <input type="date" name="paymentDate" defaultValue={todayEastern()} className={inputStyle} />
                </label>
              </div>

              {pendingExtra > 0 ? (
                <label className="block">
                  <span className={labelStyle}>Extra Payment Type</span>
                  <select name="extraType" className={inputStyle} defaultValue="Extra Payment">
                    <option>Extra Payment</option><option>Tip / Bonus</option><option>Credit</option><option>Other</option>
                  </select>
                </label>
              ) : null}

              <label className="block">
                <span className={labelStyle}>Payment Note</span>
                <textarea name="paymentNote" rows={2} placeholder="Optional" className={`${inputStyle} resize-y`} />
              </label>

              <button type="submit" className="w-full rounded-xl bg-[#647d5b] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#56683f]">Save Payment & Mark Paid</button>
            </form>
          </div>
        </div>
      ) : null}
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-4 space-y-3">
              ...
              <form action={deleteService} onSubmit={confirmDelete}>
                <input type="hidden" name="clientId" value={clientId} /><input type="hidden" name="serviceId" value={service.id} />
                <button type="submit" className="w-full rounded-xl border border-[#e2c6c2] px-4 py-2.5 text-sm font-semibold text-[#9a554d] transition hover:bg-[#fbefed]">Delete Service</button>
              </form>
            </div>
          </div>
        </div>
