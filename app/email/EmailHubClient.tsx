...
type SentGroup = { id: string; messages: SentEmail[]; sentAt: string; subject: string; body: string };

const HTML_TEMPLATE_PREFIX = "__JGO_HTML__";
...
export default function EmailHubClient({ contacts, initialEmailContacts, initialTemplates, initialSent }: { contacts: Contact[]; initialEmailContacts: EmailContact[]; initialTemplates: Template[]; initialSent: SentEmail[] }) {
  ...
  const sentGroups = useMemo<SentGroup[]>(() => {
    const groups: SentGroup[] = [];
    const ordered = [...sent].sort((a, b) => new Date(b.sent_at).getTime() - new Date(a.sent_at).getTime());
    for (const message of ordered) {
      const messageTime = new Date(message.sent_at).getTime();
      const group = groups.find((candidate) => Math.abs(new Date(candidate.sentAt).getTime() - messageTime) <= 12000);
      if (group) group.messages.push(message);
      else groups.push({ id: `sent-${message.id}`, messages: [message], sentAt: message.sent_at, subject: message.subject, body: message.body });
    }
    return groups;
  }, [sent]);
  ...
  function insertFirstNameGreeting() { insertHtmlAtCursor("{{first_name}},<br><br>"); }

  function insertButton() {
    const buttonText = window.prompt("Button text", "Leave a Review")?.trim();
    if (!buttonText) return;
    let url = window.prompt("Button link", "https://jgohire.com/leaveareview")?.trim();
    if (!url) return;
    if (!/^https?:\/\//i.test(url)) url = `https://${url}`;
    try { new URL(url); } catch { setNotice("Please enter a valid button link."); return; }
    const safeText = buttonText.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
    const safeUrl = url.replaceAll("&", "&amp;").replaceAll('"', "&quot;").replaceAll("'", "&#039;");
    insertHtmlAtCursor(`<div style="margin:22px 0;"><a href="${safeUrl}" target="_blank" style="display:inline-block;background:#647d5b;color:#ffffff;text-decoration:none;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:700;line-height:1;padding:14px 24px;border-radius:999px;">${safeText}</a></div><br>`);
    setNotice(`Added “${buttonText}” button.`);
  }
  ...
  function composeTo(email: string, name = "", clientId: number | null = null) {
    resetDraft(); setRecipients([{ email: email.toLowerCase(), name, clientId }]); setTab("compose"); setNotice(name ? `New email to ${name}.` : `New email to ${email}.`);
  }

  function useTemplate(template: Template) {
    const storedAsHtml = template.body.startsWith(HTML_TEMPLATE_PREFIX);
    const html = storedAsHtml ? template.body.slice(HTML_TEMPLATE_PREFIX.length) : plainToHtml(template.body);
    const plain = storedAsHtml ? stripHtml(html) : template.body;
    setSubject(template.subject); setBody(plain); setBodyHtml(html); setActiveTemplateId(template.id); setTab("compose");
  ...
  function removeTemplate(id: number) { startTransition(async () => { const result = await deleteEmailTemplate(id); if (!result.ok) return setNotice(result.error || "Could not delete template."); setTemplates((current) => current.filter((item) => item.id !== id)); setNotice("Template deleted."); }); }
  ...
  const tabs: { id: Tab; label: string }[] = [{ id: "compose", label: "Compose" }, { id: "contacts", label: `Contacts (${initialEmailContacts.length})` }, { id: "templates", label: "Templates" }, { id: "sent", label: sent.length ? `Sent (${sent.length})` : "Sent" }];
