...
export default function EmailHubClient({ contacts, initialEmailContacts, initialTemplates, initialSent }: { contacts: Contact[]; initialEmailContacts: EmailContact[]; initialTemplates: Template[]; initialSent: SentEmail[] }) {
  ...
  const sentGroups = useMemo<SentGroup[]>(() => {
    ...
    for (const message of ordered) {
      ...
      const group = groups.find((candidate) => {
        const groupTime = new Date(candidate.sentAt).getTime();
        return Math.abs(groupTime - messageTime) <= 12000;
      });
  ...
  function insertFirstNameGreeting() {
    ...
    if (range && editor.contains(range.commonAncestorContainer)) {
      ...
      const fragment = range.createContextualFragment("{{first_name}},<br><br>");
      ...
      editor.insertAdjacentHTML("beforeend", "{{first_name}},<br><br>");
      const nextRange = document.createRange();
  ...
  function saveTemplate() { startTransition(async () => { const result = await createEmailTemplate({ name: templateName, subject, body }); if (!result.ok || !result.template) return setNotice(result.error || "Could not save template."); setTemplates((current) => [...current, result.template as Template]); setTemplateName(""); setNotice("Template saved to JGO OS."); }); }
      ...
      <div className="mx-auto max-w-7xl p-6 lg:p-10">
        ...
        {tab === "sent" && <div><div className="mb-6"><h2 className="text-2xl font-bold">Sent History</h2><p className="mt-1 text-sm text-[#708075]">Each send appears once. If you sent the same email to multiple people, expand it to see every recipient.</p></div>{sentGroups.length === 0 ? <div className="rounded-3xl border border-dashed bg-white/60 p-12 text-center"><p className="font-semibold">No emails sent yet.</p></div> : <div className="space-y-3">{sentGroups.map((group) => {
          ...
          return <details key={group.id} className="group overflow-hidden rounded-3xl border border-[#dfe6db] bg-white shadow-sm" open={!multiple}>
            <summary className={`list-none p-5 ${multiple ? "cursor-pointer" : "cursor-default"}`}>
              <div className="grid gap-4 md:grid-cols-[220px_1fr_180px] md:items-start">
                <div>{multiple ? <><strong className="block text-sm text-[#3f5140]">{group.messages.length} recipients</strong><span className="mt-1 block text-xs font-semibold text-[#647d5b]">View recipients ↓</span></> : <button type="button" onClick={(event) => { event.preventDefault(); setSelectedEmail(first.recipient_email); setTab("contacts"); }} className="text-left"><strong className="block text-sm hover:text-[#647d5b]">{first.recipient_name || first.recipient_email}</strong><span className="text-xs text-[#7b887d]">{first.recipient_email}</span></button>}</div>
                <div><strong className="block text-sm">{group.subject}</strong><p className="mt-1 line-clamp-2 whitespace-pre-line text-xs leading-5 text-[#7b887d]">{group.body}</p></div>
                <span className="text-xs text-[#8b948c] md:text-right">{new Date(group.sentAt).toLocaleString()}</span>
              </div>
            </summary>
