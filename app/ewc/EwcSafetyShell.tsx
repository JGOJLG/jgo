...
export default function EwcSafetyShell({ children }: { children: ReactNode }) {
  ...
  function protectDelete(event: MouseEvent<HTMLDivElement>) {
    ...
    const looksLikeDelete = text === "×" || text === "✕" || text === "delete" || label.includes("delete") || title.includes("delete");
    if (!looksLikeDelete) return;

    const confirmed = window.confirm("Are you sure you want to delete this finance row? This action removes the row from the tracker.");
    if (!confirmed) {
  ...
  return (
    <div onClickCapture={protectDelete} onInputCapture={markSavedSoon}>
