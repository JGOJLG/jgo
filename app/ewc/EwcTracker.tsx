...
export default function EwcTracker({ initialEntries }: { initialEntries:EwcEntry[] }) {
  ...
  function table(section:EwcEntryType,rows:EwcEntry[]){const title=section==="Session"?"Sessions":section;return <section className="relative min-w-0 max-w-full rounded-2xl border border-[#dfe6db] bg-white shadow-sm">
    <div className="flex items-center justify-between rounded-t-2xl border-b border-[#dfe6db] bg-[#fbfaf6] px-5 py-4"><div><h2 className="text-xl font-bold text-[#243128]">{title}</h2><p className="mt-1 text-sm text-[#708075]">Newest entries stay at the top. All edits auto-save.</p></div><button type="button" onClick={()=>addRow(section)} disabled={isPending} className="shrink-0 rounded-xl bg-[#647d5b] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">+ Add {section==="LinkedIn"?"LinkedIn Client":section}</button></div>
    <div className="w-full max-w-full overflow-x-auto overscroll-x-contain touch-pan-x"><div className="min-w-[1320px]">
      <div className="sticky top-0 z-40 grid grid-cols-[44px_190px_92px_135px_105px_105px_110px_110px_110px_1fr_38px] border-b border-[#dfe6db] bg-[#eef2ea] text-[9px] font-bold uppercase tracking-[0.08em] text-[#647066] shadow-[0_4px_10px_rgba(67,78,69,0.10)]">{["#","Client","Date","Service","Owed","Paid","Received","Deduction","Final Earned","Notes",""].map((x,i)=><div key={`${x}-${i}`} className="border-r border-[#dfe6db] px-2 py-2.5 text-center">{x}</div>)}</div>
      {rows.length===0?<div className="p-10 text-center text-sm text-[#708075]">No entries yet.</div>:null}
      {rows.map((row,index)=>{const outstanding=isOutstanding(row);return <div key={row.id} onDragOver={e=>e.preventDefault()} onDrop={()=>reorder(section,row.id)} className={`group grid grid-cols-[44px_190px_92px_135px_105px_105px_110px_110px_110px_1fr_38px] border-b ${outstanding?"border-[#e7d8d5] bg-[#fbf5f4] shadow-[inset_3px_0_0_#b98f89]":index%2?"border-[#edf0ea] bg-[#fcfdfb]":"border-[#edf0ea] bg-white"}`}>
        ...
        <div className={outstanding?"bg-[#f3e7e5] font-bold text-[#815e59]":""}><MoneyInput value={Number(row.amount_owed||0)} onCommit={v=>{updateLocal(row.id,"amount_owed",v);persist(row,{amount_owed:v});}}/></div>
    ...
    <div className="min-w-0 max-w-full space-y-7 p-4 sm:p-6 lg:p-10">
      <section className="w-full rounded-2xl border border-[#dfe6db] bg-white p-6 shadow-sm lg:p-7">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-[#7f8d82]">Finance</p><p className="mt-2 text-4xl font-bold text-[#56754f]">{money(finance.final)}</p><p className="mt-1 text-xs font-medium text-[#879188]">Final earned · {finance.label}</p></div>
          <div className="inline-flex w-fit max-w-full overflow-x-auto rounded-xl border border-[#dfe6db] bg-[#f7f8f3] p-1">{(["total","week","month","year"] as FinanceView[]).map(v=><button key={v} type="button" onClick={()=>setFinanceView(v)} className={`shrink-0 rounded-lg px-4 py-2 text-sm font-semibold capitalize transition ${financeView===v?"bg-white text-[#4f6b49] shadow-sm":"text-[#7b877e] hover:text-[#4f6b49]"}`}>{v}</button>)}</div></div>
        <div className="mt-6 grid grid-cols-2 gap-y-4 border-t border-[#e7ebe4] pt-5 sm:grid-cols-5 sm:divide-x sm:divide-[#e7ebe4]">{[["Owed",finance.owed],["Paid",finance.paid],["Received",finance.received],["Deductions",finance.deductions],["Paid Entries",finance.paidCount]].map(([l,v],i)=><div key={String(l)} className={i?"sm:px-4":"sm:pr-4"}><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#89938b]">{l}</p><p className={`mt-1 text-lg font-bold ${l==="Deductions"?"text-[#a06d62]":l==="Owed"&&Number(v)>0?"text-[#815e59]":"text-[#334139]"}`}>{l==="Paid Entries"?String(v):money(Number(v))}</p></div>)}</div>
      </section>
      {table("Session",sessions)}{table("LinkedIn",linkedin)}{table("Other",other)}
    </div>
    <div className={`fixed bottom-5 right-5 z-50 rounded-full px-4 py-2 text-sm font-semibold text-white shadow-lg transition ${savedMessage?"translate-y-0 bg-[#647d5b] opacity-100":"pointer-events-none translate-y-2 opacity-0"}`}>{savedMessage}</div>
  </section>;
}