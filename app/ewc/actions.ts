...
export async function createEwcEntry(section: EwcEntryType) {
  ...
  const { data: firstRow } = await supabase.from("ewc_entries").select("sort_order").eq("section", section).order("sort_order", { ascending: true }).limit(1).maybeSingle();
  const { data, error } = await supabase.from("ewc_entries").insert({
    section,
    client_name: "",
    service_date: null,
    service_type: section === "Session" ? "Session" : section === "LinkedIn" ? "LinkedIn" : "Other",
    amount_owed: 0,
    amount_paid: 0,
    amount_received: 0,
    stripe_fee: 0,
...
export async function updateEwcEntry(formData: FormData) {
  ...
  const { error } = await supabase.from("ewc_entries").update({
    ...
    amount_received: cleanNumber(formData.get("amount_received")),
...
export async function updateEwcMoved(id: number, moved: boolean) {
  const supabase = await createClient();
  if (!id) throw new Error("EWC entry ID is required.");
  const { error } = await supabase.from("ewc_entries").update({ moved, updated_at: new Date().toISOString() }).eq("id", id);
  if (error) throw new Error(error.message);
  revalidatePath("/ewc");
}
...
export async function reorderEwcEntries(section: EwcEntryType, orderedIds: number[]) {
  const supabase = await createClient();
  const results = await Promise.all(orderedIds.map((id, index) => supabase.from("ewc_entries").update({ sort_order: index + 1, updated_at: new Date().toISOString() }).eq("id", id).eq("section", section)));
